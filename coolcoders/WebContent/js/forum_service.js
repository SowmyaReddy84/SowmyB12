...
angular.module('myApp').factory('ForumService', ['$http', '$q', function($http, $q){
    ...
    function fetchAllForums() {
        ...
        $http.get('http://localhost:8181/CoolCodersBackend/forums')
            .then(
            function (response) {
    ...
    function addForum(forumm) {
        ...
        $http.post('http://localhost:8181/CoolCodersBackend/frmadd/insert/',forumm)
            .then(
    ...
    function addRpl(reply) {
        var deferred = $q.defer();
        $http.post('http://localhost:8181/CoolCodersBackend/reply/insert/',reply)
    ...
    function  fetchAllRep(fid) {
        var deferred = $q.defer();
        $http.get('http://localhost:8181/CoolCodersBackend/replies/'+fid)
            .then(
            function (response) {
            	console.log('Got data');
            	deferred.resolve(response.data);
